...
data <- raster::raster("./09_monochrome/data/srtm_51_09.tif")
...
dmat %>%
  sphere_shade(texture = "bw",sunangle = 10) %>%
  add_water(detect_water(dmat), color = "imhof4") %>%
  add_shadow(ray_shade(dmat, zscale = 10), 0.5) %>%
  # add_shadow(ambient_shade(dmat), 0.5) %>%
